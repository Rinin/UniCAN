 ...
 */
static unican_node* unican_allocate_node(uint16_t msg_id, uint16_t from, uint16_t to, uint16_t data_len)
{
	if (state.is_online == UNICAN_ENABLED)
	{
		...
		else
		{
			...
			temp_msg->data = (uint8_t*)malloc (data_len);
 ...
 */
static unican_buffer* unican_find_buffer (uint16_t address_from)
{
	uint16_t i;
...
static uint16_t can_check_message (can_message* msg)
{
...
static void unican_collect_buffer(unican_buffer* buff)
{
	...
	{
		int len;
		uint16_t crc_buffered, crc_calculated;
		len = buff->node->value->unican_length;
		crc_buffered = buff->node->value->data[len-2] + buff->node->value->data[len-1]*256;
		crc_calculated = crc16(buff->node->value->data, len-2);
		if (crc_buffered == crc_calculated)
...
void can_receive_message (can_message* msg)
{
	...
	if (state.is_online == UNICAN_ENABLED)
	{
		...
		uint16_t address_to = 0;
		uint16_t address_from = 0;
		uint8_t data_bit = 0;
		uint16_t i;
		...
		else
		{
			...
			if (msg->can_dlc >= CAN_MIN_DLC)
				msg_id = msg->data[0] + (msg->data[1] << 8);
			else
			{
			...
			case UNICAN_START_LONG_MESSAGE:
			{
				...
				else
				{
					...
					uint16_t msg_subid = msg->data[2] + (msg->data[3] * 256);
					uint16_t data_length = msg->data[4] + (msg->data[5] * 256);
					if (data_length < 2) {
						unican_error(UNICAN_WRONG_CRC);
						return;
					}
...
static void can_set_identifier(unican_message* msg, can_message* can_buff, uint8_t data_bit)
{
		...
		else
		{
			...
			if (( i % 8) > 0)//something left
			{
				...
				if ((i % 8) < 7) {
					...
					can_buff.data[7] = UINT16RIGHT ( crc );
					can_buff.can_dlc=8;
					can_send_message(&can_buff);
					can_buff.data[0] = UINT16LEFT ( crc );
					can_buff.can_dlc=1;
					can_send_message (&can_buff);
				}
			}
			else{
				can_buff.data[0] = UINT16RIGHT ( crc );
				can_buff.data[1] = UINT16LEFT ( crc );
				can_buff.can_dlc=2;
				can_send_message (&can_buff);
			}
		}
